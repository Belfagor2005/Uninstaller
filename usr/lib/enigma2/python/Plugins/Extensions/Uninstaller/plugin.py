...
def pakage_entry(name):
    ...
    pngs = resolveFilename(
        SCOPE_PLUGINS,
        "Extensions/{}/pic.png".format('Uninstaller'))
    if screenwidth.width() == 2560:
        res.append(
...
class Uninstaller(Screen):
    skin = '''
            <screen name="Uninstaller" position="center,center" size="1920,1080" title="Uninstaller by Lululla">
                <widget name="list" position="35,156" size="1900,890" scrollbarMode="showOnDemand" zPosition="2" />
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(
            ['OkCancelActions'], {
                'ok': self.okClicked, 'cancel': self.close}, -1)
        txt = _('Wait Please...')
        ...
        with open(myfile, 'w') as f:
            for root, dirs, files in os.walk(path):
                if files is not None:
                    for name in files:
                        if name.startswith(
                                'enigma2-plugin') and name.endswith('.list'):
                            print(str(name))
    ...
    def okClicked(self):
        ...
        if ires is not None:
            ...
            self.session.openWithCallback(
                self.test, ChoiceBox, title=_('Select method:'), list=[
                    (_('Remove'), 'rem'), (_('Force Remove'), 'force')])
        else:
            return
        return
