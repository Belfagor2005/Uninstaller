...
TMPPATH=/tmp/Uninstaller-install
FILEPATH=/tmp/Uninstaller-main.tar.gz

echo "Starting Uninstaller installation..."

if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/Uninstaller
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/Uninstaller
fi
...
cleanup() {
    ...
    [ -d "/tmp/Uninstaller-main" ] && rm -rf "/tmp/Uninstaller-main"
}
...
wget --no-check-certificate 'https://github.com/Belfagor2005/Uninstaller/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "Failed to download Uninstaller package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "Failed to extract Uninstaller package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/Uninstaller-main/usr/lib/enigma2/python/Plugins/Extensions/Uninstaller" ]; then
    cp -r "$TMPPATH/Uninstaller-main/usr/lib/enigma2/python/Plugins/Extensions/Uninstaller"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from standard plugin directory"
elif [ -d "$TMPPATH/Uninstaller-main/usr/lib64/enigma2/python/Plugins/Extensions/Uninstaller" ]; then
    cp -r "$TMPPATH/Uninstaller-main/usr/lib64/enigma2/python/Plugins/Extensions/Uninstaller"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/Uninstaller-main/usr" ]; then
    cp -r "$TMPPATH/Uninstaller-main/usr"/* /usr/ 2>/dev/null
...
#########################################################
#     Uninstaller $version INSTALLED SUCCESSFULLY       #
